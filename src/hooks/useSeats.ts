/**
 * @file useSeats.ts
 * @description Custom hook for managing the state of seats, including initialization and updates.
...
const calculateDisplayLabelForSeat = (
  ...
  if (!section.rowPatterns || !section.rowPatterns[targetGridRow]) {
    const displayRowLetter = String.fromCharCode(
      65 + section.rows - 1 - targetGridRow,
    );
  ...
  for (const patternItem of section.rowPatterns[targetGridRow]) {
    if (patternItem.type === 'seats') {
      ...
      if (
        targetGridCol >= currentGridColPointer &&
        targetGridCol < currentGridColPointer + patternItem.count
      ) {
        return '';
      }
...
export const useSeats = (
  initialLayoutConfig: SeatingLayoutProps['initialLayoutConfig'],
  dbName: string = 'SeatingDB',
) => {
    ...
    dbPromiseRef.current = openDB(dbName, 1, {
      upgrade(db) {
        if (!db.objectStoreNames.contains('layouts')) {
          console.log(
            'Creating layouts object store in IndexedDB via useSeats',
          );
  ...
  const initializeSeats = useCallback(() => {
    console.log(
      'useSeats: Attempting to initialize seats with config:',
      initialLayoutConfig,
    );
    const initialSeatsMap: Record<string, Seat> = {};
    if (!initialLayoutConfig || !initialLayoutConfig.sections) {
      console.error(
        'useSeats: Cannot initialize seats: initialLayoutConfig or sections are missing.',
      );
      setSeats({});
      return;
    }
  ...
  useEffect(() => {
    console.log(
      'useSeats: useEffect for loading seats triggered. initialLayoutConfig or initializeSeats changed.',
    );
    (async () => {
      try {
        ...
        if (
          savedLayout &&
          typeof savedLayout.seats === 'object' &&
          savedLayout.seats !== null &&
          Object.keys(savedLayout.seats).length > 0
        ) {
          const firstSavedKey = Object.keys(savedLayout.seats)[0];
          const firstConfigSectionId = initialLayoutConfig?.sections?.[0]?.id;
          if (
            firstSavedKey &&
            firstConfigSectionId &&
            firstSavedKey.startsWith(firstConfigSectionId + '-')
          ) {
            loadSavedData = true;
          } else {
            console.warn(
              'useSeats: Saved IndexedDB data key format appears to mismatch current layout config. Re-initializing.',
            );
          }
        }
        ...
        if (loadSavedData) {
          ...
          console.log(
            'useSeats: No saved layout, empty seats in DB, or key mismatch. Calling initializeSeats().',
          );
          initializeSeats();
        }
      } catch (error) {
        console.error(
          'useSeats: Failed to load seats from DB, calling initializeSeats() as fallback.',
          error,
        );
        initializeSeats();
      }
  ...
  const saveSeatsToDB = useCallback(
    async (seatsData: Record<string, Seat>) => {
      try {
        const db = await dbPromise;
