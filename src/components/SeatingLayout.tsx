/**
 * @file SeatingLayout.tsx
 * @description Main component for displaying the seating layout, handling seat selection, and booking.
...
const SeatingLayout: React.FC<SeatingLayoutProps> = ({
  ...
  const getSeatInfo = useCallback(
    (seatKey: string) => {
      ...
      if (seat) {
        section = initialLayoutConfig.sections.find(
          (s: Section) => s.id === seat.sectionId,
        );
        if (section) {
  ...
  if (
    !initialLayoutConfig ||
    !initialLayoutConfig.sections ||
    !initialLayoutConfig.seatTypes
  ) {
    return (
      <div className='text-red-500'>
        Error: Initial layout configuration is missing or incomplete.
      </div>
    );
  }
  ...
  if (
    ...
    initialLayoutConfig.sections.length > 0
  ) {
        ...
        {initialLayoutConfig.sections.map((section: Section) => (
          ...
          >
            <h2 className='text-lg mb-2 text-gray-600 font-semibold'>
              {section.name} (Rs.{' '}
              {initialLayoutConfig.seatTypes[section.seatType]?.price})
            </h2>
            <div className='border p-1 inline-block'>
              {Array.from({ length: section.rows }).map((_, rowIndex) => {
                const displayRowIdentifier = String.fromCharCode(
                  65 + section.rows - 1 - rowIndex,
                );
                return (
                  <div
                  ...
                  >
                    ...
                    {Array.from({ length: section.cols }).map((_, colIndex) => {
                      ...
                      const currentSeatType = seat.displayLabel
                        ? seatType
                        : undefined;
                      ...
                      return (
                        <SeatDisplayComponent
                          ...
                          onClick={() =>
                            handleSeatClick(section.id, seat.row, seat.col)
                          }
