/**
 * @file SeatDisplay.tsx
 * @description Component for rendering an individual seat with its status and interactions.
...
const SeatDisplayComponent: React.FC<SeatProps> = ({
  status,
  icon: Icon,
  color,
  onClick,
  ...
  if (isGap) {
    ...
    return (
      <div style={{ width: 30, height: 30, margin: 4, visibility: 'hidden' }} />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ lineHeight: hasIcon ? '0.8rem' : 'normal' }}>
        {displayLabel}
      </div>
    </div>
  );
